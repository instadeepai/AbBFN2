...
@dataclass
class TimeScheduleFn(ABC):
    """An abstract base class for defining the time schedule for sampling a BFN."""

    @abstractmethod
    def __call__(self, i: int, num_steps: int) -> tuple[float, float]:
...
@dataclass
class LinearScheduleFn(TimeScheduleFn):
